...
def select_general(cls):
    ...
    # directly in mongodb, we'd say collection=db[whatever collection it is]
    ...
    while True:
        ...
        for column in [col[0] for col in index_info[chosen_index]['key']]:
            ...
            if type(attribute).__name__ == 'ReferenceField':
                ...
                # figure out the class that we're referencing.
                referenced_class = attribute.document_type
                # find an instance of the referenced parent
                target = select_general(referenced_class)
                # add the filter to point to the selected document in the parent collection.
...
def unique_general(instance):
    ...
    for constraint in constraints:
        ...
        for column in constraint['columns']:
            # convert from the column name to the attribute name.
            attribute_name = get_attr_from_column(cls, column)
